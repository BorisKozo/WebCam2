...
namespace CatenaLogic.Windows.Presentation.WebcamPlayer
{
  public class CapPlayer : Image, IDisposable
  {
    ...
    public static readonly DependencyProperty FlipProperty =
        DependencyProperty.Register("Flip", typeof(bool), typeof(CapPlayer), new PropertyMetadata(false, new PropertyChangedCallback(FlipProperty_Changed)));
    ...
    public BitmapSource CurrentBitmap
    {
      get
      {
        ...
        if (Flip)
          return new TransformedBitmap(Device.BitmapSource.Clone(), new ScaleTransform(1,-1,Device.BitmapSource.Width/2, Device.BitmapSource.Height/2));

        return Device.BitmapSource.Clone();
